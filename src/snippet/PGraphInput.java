...
public class PGraphInput {
     ...
     */
    public static Hashtable LoadSimpleGraph(PSimpleGraph newgraph) {
        System.out.print("Please enter the full path and file name for the input data: ");
        String userinput;
        userinput = PKeyboardReader.readString();
     ...
     * @returns a hash table of (String, Vertex) pairs
     */
    public static Hashtable LoadSimpleGraph(PSimpleGraph newgraph, String pathandfilename) {
        BufferedReader inbuf = PInputLib.fopen(pathandfilename);
        System.out.println("Opened " + pathandfilename + " for input.");
        String line = PInputLib.getLine(inbuf); // get first line
        StringTokenizer sTok;
        int n, linenum = 0;
        Hashtable table = new Hashtable();
        PSimpleGraph sg = newgraph;
        ...
        while (line != null) {
            ...
            if (n == 3) {
                ...
                v1 = (PVertex) table.get(v1name);
                v1Copy = (PVertex) table.get(v1CopyName);
                if (v1 == null) {
//                    System.out.println("New vertex " + v1name);
                ...
                v2 = (PVertex) table.get(v2name);
                v2Copy = (PVertex) table.get(v2CopyName);
                if (v2 == null) {
...
//              System.out.println("Inserting edge (" + v1name + "," + v2name + ")" + edgedata);
            ...
            line = PInputLib.getLine(inbuf);
        }

        PInputLib.fclose(inbuf);
